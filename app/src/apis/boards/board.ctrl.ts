...
const process = {
  createByCategoryName: async (req : Request, res : Response) => {
  ...
  findAllByCategoryNum: async (req : Request, res : Response) => {
    const board = new Board(req);
  ...
  findOneByNum: async (req : Request, res : Response) => {
    const board = new Board(req);
  ...
  updateOnlyHit: async (req : Request, res : Response) => {
    const board = new Board(req);
  ...
  updateOnlyStatus: async (req : Request, res : Response) => {
    const board = new Board(req);
    ...
    logger.error(`PATCH /api/boards/categoryName/num/status 400 ${response.msg}`);
    return res.status(400).json(response);
  },
