...
const process = {
  createByBoardNum: async (req : Request, res : Response) => {
  ...
  createReplyByGroupNum: async (req : Request, res : Response) => {
    const comment = new Comment(req);
    ...
    logger.info(`POST /api/boards/categoryName/num/groupNum 400 ${response.msg}`);
    return res.status(400).json(response);
  },

  updateByNum: async (req : Request, res : Response) => {
    const comment = new Comment(req);
  ...
  deleteByNum: async (req : Request, res : Response) => {
    const comment = new Comment(req);
    ...
    logger.info(`DELETE /api/boards/categoryName/num/commentNum 400 ${response.msg}`);
    return res.status(400).json(response);
  },

  findStudentIdByNum: async (req : Request, res : Response) => {
    const comment = new Comment(req);
    ...
    logger.info(`GET /api/boards/categoryName/num.comments 400 ${response.msg}`);
    return res.status(400).json(response);
  },
};
