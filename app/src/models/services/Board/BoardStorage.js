...
class BoardStroage {
  ...
  static findAllByCategoryNum(categoryNum, lastNum) {
    ...
    return new Promise((resolve, reject) => {
      const query = `SELECT bo.no AS num, st.id AS studnetId, st.nickname AS nickname, st.profile_path AS profilePath, bo.thumbnail, bo.title, bo.hit, bo.price, bo.status,
