...
docker stop idu-market-server
docker rm idu-market-server
# 저는 Docker를 이용할 것이기 때문에, 돌아가고 있는 Docker Container를 중지시키고, 제거합니다.
...
if [[ "$(docker images -q dnfla960/idu-market-server:latest 2> /dev/null)" != "" ]]; then
docker rmi -f $(docker images --format '{{.Repository}}:{{.Tag}}' --filter=reference='dnfla960/idu-market-server:latest')
fi
